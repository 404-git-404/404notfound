...
仓库根目录的 `smartdns.conf` 是唯一受维护的 SmartDNS 配置基准模板。为保持脚本可单文件运行，`404notfound.sh` 和 `scripts/update-smartdns.sh` 各自内嵌该模板的逐字节镜像；CI 会提取两处 heredoc 并分别与根模板执行 `cmp`，防止配置漂移。SmartDNS 只监听 `127.0.0.1:53` 的 UDP/TCP，保留持久缓存、serve-expired、prefetch、`tcp:443,ping` 测速、`first-ping` 响应策略和双栈选择。
...
更新器完全自包含，不需要克隆仓库。以 root 身份直接运行：
