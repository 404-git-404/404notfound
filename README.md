...
SmartDNS 模板完整内嵌在 `404notfound.sh`：只监听 `127.0.0.1:53` 的 UDP/TCP，启用持久缓存、serve-expired、prefetch、`tcp:443,ping` 测速和 `first-ping` 响应策略，保留统一上游。安装后使用 `smartdns -v` 取得并记录真实版本，只有命令成功且输出非空才继续。
